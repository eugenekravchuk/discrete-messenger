...
class RSA:
    ...
    def setkeys(self, prime_list):
        ...
        """
        prime1 = self.pickrandomprime(prime_list)
        prime2 = self.pickrandomprime(prime_list)
    ...
    def encoder(self, message, pub_key, n):
        """
        ...
        """
        return [self.encrypt(ord(letter), pub_key, n) for letter in message]


    def decoder(self, encoded, pri_key, n):
        """
        ...
        s = ''
        for num in encoded:
            s += chr(self.decrypt(num, pri_key, n))
        return s
